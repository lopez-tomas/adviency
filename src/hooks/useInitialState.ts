...
const useInitialState = () => {
  ...
  const addGift = (payload: AddGiftDto): string => {
    ...
    return `Gift ${newGift.gift} created successfully`;
  }
  ...
  const editGift = (id: Gift['id'], payload: EditGiftDto): string => {
    ...
    if (index === undefined) return `Gift with ID #${id} not found.`;

    const newGift = {...state.gifts[index], ...payload};
  ...
  const deleteAll = (): string => {
    ...
    return `All gifts deleted successfully.`;
  }
